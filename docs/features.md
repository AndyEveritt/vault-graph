...
<!-- gallery-nav:start (generated by scripts/gallery-nav.mjs -- do not edit by hand) -->
> **New in 2.9.0** — [Folder order](#folder-order)
<!-- gallery-nav:end -->

| | |
|---|---|
| **The disc** | [The disc, growing](#the-disc-growing) · [Subfolders](#subfolders) · [Reading each folder's share](#reading-each-folders-share) · [Unlinked notes join their folder](#unlinked-notes-join-their-folder) · [Folder order](#folder-order) |
...
| **Time** | [The timeline](#the-timeline) · [The heatmap](#the-heatmap) · [The recent lens](#the-recent-lens) · [Compact date axis](#compact-date-axis) |
...
Wedges and the legend run in name order by default. Switch **Folder order** in the gear
menu to **File explorer** and they follow your vault's own
[Custom File Explorer Sorting](https://github.com/SebastianMC/obsidian-custom-sort) spec
instead — pinned names first, then whatever ascending or descending order the spec sets —
or to **Size**, biggest folder first. Root notes take their place among the folders under
either mode rather than always sitting first or last. Only the top-level wedges and the
legend follow the chosen order; a folder's own subfolders stay ordered by size, which is
what gives its fan of tints their gradient.

<img src="https://raw.githubusercontent.com/luke321/vault-graph/main/assets/features/sort.webp" width="100%" alt="Settings opened, Folder order switched from Name to File explorer so the wedges and the legend reorder to match the vault's own custom-sort spec, then switched back to Name">
...
Three chips under the band — Today, Last 7 and Since last open — halo the notes from a
recent window wherever they sit on the disc, and dim everything else a little. Nothing
moves. Last 7 is a rolling seven days, Since last open appears only inside Obsidian, and
the band counts either date, Added or Touched, with every chip saying which one it means.

<img src="https://raw.githubusercontent.com/luke321/vault-graph/main/assets/features/recent.webp" width="100%" alt="Today clicked and the notes added that day haloed while the rest of the disc dims, then Last 7 widening the window, the band switched to count by Touched and back to Added, and the chip clicked again to let the lens go">
